...
static uint32_t SD_PowerON(SD_HandleTypeDef *hsd)
{
  ...
  while((count < SDMMC_MAX_VOLT_TRIAL) && (validvoltage == 0U))
  {
    ...
    /* Send CMD41 */
    /* PATCH 2026-05-22: bỏ SD_SWITCH_1_8V_CAPACITY (S18R) — STM32F4 SDIO không hỗ trợ
     * 1.8V signaling, nếu để bit này thẻ UHS-I (vd SanDisk Extreme Pro 128GB) sẽ chờ
     * CMD11 và treo init. Buộc thẻ ở 3.3V để F4 driver được. NOTE: patch này sẽ mất nếu
     * cập nhật HAL hoặc regen CubeMX — phải apply lại. */

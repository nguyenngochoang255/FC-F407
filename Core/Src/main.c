...
/* Nguong an toan va mapping kenh RC tu receiver CRSF. */
#define MIN_ARM 250

#define ARM_BLOCK_NONE       0U
#define ARM_BLOCK_SWITCH_OFF 1U
#define ARM_BLOCK_WAIT_EDGE  2U
#define ARM_BLOCK_THROTTLE   3U
#define ARM_BLOCK_GYRO       4U
#define ARM_BLOCK_RC_LOST    5U
#define ARM_BLOCK_BAD_RX     6U
#define ARM_REQUIRE_SWITCH_CYCLE 0U

#define DISARM_REASON_NONE        0U
#define DISARM_REASON_SWITCH_OFF  1U
#define DISARM_REASON_BAD_RX      2U
#define DISARM_REASON_RC_LOST     3U
#define DISARM_REASON_WAIT_ARM    4U
#define DISARM_REASON_THROTTLE    5U
#define DISARM_REASON_GYRO        6U
#define DISARM_REASON_BAD_STATE   7U

#define ARM_THROTTLE_MAX     350      
/* Qua 1.5 s khong co frame CRSF hop le thi coi la mat RC. */
#define RC_FAILSAFE_TIMEOUT  1500000U 


#define GESTURE_THROTTLE_MAX  300
#define GESTURE_YAW_MAX       300
#define GESTURE_PITCH_MAX     300
#define GESTURE_ROLL_MIN      1700
#define GESTURE_DEBOUNCE_US   200000U
...
/* Mapping kenh CRSF: CH0 roll, CH1 pitch, CH2 throttle, CH3 yaw, CH4 arm. */
#define RX_ROLL_CH            0U   
#define RX_PITCH_CH           1U   
#define RX_THROTTLE_CH        2U   
#define RX_YAW_CH             3U   
#define RX_ARM_CH             4U   
#define RX_FLIGHT_SW_CH       5U   
#define RX_ALT_HOLD_CH        8U
#define RX_ALT_HOLD_BLOCK_CH  10U


/* SPI doc IMU 13 byte = 1 byte dia chi bat dau + 12 byte du lieu accel/gyro. */
volatile uint8_t rx_dma[13];
volatile uint8_t tx_dma[13];
...
/* FSM doc MS5611: moi chu ky doc ap suat D1 va nhiet do D2 bang DMA. */
typedef enum {
    MS5611_S_START_D1,
...
volatile uint8_t baro_dma_done = 0;        
uint32_t baro_convert_start_us = 0;
pt1Filter_t baro_alt_lpf;
float32_t baro_pressure_ref_pa = 0.0f;
uint8_t  baro_ref_locked = 0;
/* Lay trung binh 50 mau ap suat dau lam moc mat dat de tinh do cao tuong doi. */
uint16_t baro_ref_sample_n = 0;
float64_t baro_ref_sample_acc = 0.0;       
float32_t baro_alt_raw = 0.0f;
float32_t baro_alt_filtered = 0.0f;
BARO_RAW_DATA_t BARO_DATA;
uint8_t  baro_init_result = 0xFF;      
float32_t BARO_ALT_SLEW_MPS = 0.55f;       
float32_t baro_alt_sane = 0.0f;
static uint8_t baro_alt_sane_init = 0U;
static uint32_t baro_alt_sane_prev_us = 0U;

/* Observer do cao ket hop barometer va gia toc truc Z. */
float32_t vz_est   = 0.0f;
float32_t   alt_est         = 0.0f;        
float32_t az_up = 0.0f;       
uint8_t     alt_obs_init    = 0;           
float32_t   ALT_OBS_W1      = 2.1f;        
float32_t   ALT_OBS_W2      = 2.25f;       


float32_t   ALT_OBS_W3      = 1.0f;        
float32_t accel_bias_z_est = 0.0f;
float32_t   ALT_OBS_BIAS_CLAMP = 0.5f;     
...
/* Nguyen mau cac ham xu ly chinh cua firmware. */
void SystemClock_Config(void);


void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if(GPIO_Pin == INT_ICM42688P_Pin && ICM42688P.cfg.ready){
		       uint32_t prev_timestamp = ICM42688P_DATA.timestamp;
		       ICM42688P_DATA.timestamp = TIM2->CNT;
				HAL_GPIO_WritePin(CS_ICM42688P_GPIO_Port, CS_ICM42688P_Pin, GPIO_PIN_RESET);
				if(HAL_SPI_TransmitReceive_DMA(&hspi1,(uint8_t*)tx_dma,(uint8_t*)rx_dma, 13) != HAL_OK){
				   HAL_GPIO_WritePin(CS_ICM42688P_GPIO_Port, CS_ICM42688P_Pin, GPIO_PIN_SET);
				}
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi) {
    if (hspi->Instance == hspi1.Instance){
    	...
    	HAL_GPIO_WritePin(CS_ICM42688P_GPIO_Port, CS_ICM42688P_Pin, GPIO_PIN_SET);
    	for(int i = 0 ; i < 3 ; i ++){
    		raw_adc_acc_imu[i]  = (float32_t)((int16_t)((rx_dma[2*i + 1] << 8) | rx_dma[2*i + 2])) * ICM42688P.convert_factor[i];  
    		raw_adc_gyro_imu[i] = (float32_t)((int16_t)((rx_dma[2*i + 7] << 8) | rx_dma[2*i + 8])) * ICM42688P.convert_factor[i + 3] * DEG_TO_RAD; 
    ...
    case MS5611_S_WAIT_D1_CONVERT:
        if ((TIM2->CNT - baro_convert_start_us) >= 10000U) {   
            if (hi2c2.State == HAL_I2C_STATE_READY) {
                baro_dma_done = 0;
                if (MS5611_StartReadADC_DMA(&MS5611_BARO) == HAL_OK) {
...
/* Thu tu khoi tao: HAL, clock, GPIO/DMA/SPI/I2C/UART/TIM, cam bien, bo loc va PID. */
int main(void)
{
  HAL_Init();
  SystemClock_Config();
  ...
  ICM_42688_Init(&ICM42688P, &hspi1, CS_ICM42688P_GPIO_Port, CS_ICM42688P_Pin);

  /* ICM42688 SPI read: bit 7 = 1 nen OR 0x80; doc block tu ACCEL_DATA_X1. */
  tx_dma[0] = 0x80 | ICM42688_ACCEL_DATA_X1;
  rx_dma[0] = 0x00;
  for(int i = 1 ; i < 13 ; i ++){
    tx_dma[i] = 0x00;
    rx_dma[i] = 0x00;
...
void SystemClock_Config(void)
{
  ...
  /* HSE 8 MHz / PLLM 8 = 1 MHz; * PLLN 336 = VCO 336 MHz. */
  RCC_OscInitStruct.PLL.PLLM = 8;
  RCC_OscInitStruct.PLL.PLLN = 336;
  /* PLLP /2 tao SYSCLK 168 MHz; PLLQ /7 tao clock 48 MHz cho ngoai vi can 48 MHz. */
  ...
  /* HCLK=168 MHz, APB1=42 MHz, APB2=84 MHz; timer tren APB1 co timer clock 84 MHz. */
